...
namespace Zappy::GUI {
    ClientApi::ClientApi(std::string aAddress, unsigned int aPort, std::string aTeamName) : _address(const_cast<char *>(aAddress.c_str())), _port(aPort), _teamName(aTeamName), _connectStatus(-1), _readBuffer(strdup("")), _writeBuffer(strdup("")), _serverFd(-1) {}
